...
#define KEYBOARD_DEV "/dev/input/event0" //键盘
...
static int USBjoypadGet(void)
{
	...
	if (0 < read(USBjoypad_fd, &e, sizeof(e)))
	{
		if (0x2 == e.type)
		{
			...
			*/
			// if (0x8001 == e.value && 0x7 == e.number)
			// {
			// 	joypad |= 1 << 4;
			// }
			if (-32767 == e.value && 0x7 == e.number)
			{
				joypad |= 1 << 4;
			}
			/*下：
		...
		if (0x1 == e.type)
		{
			...
			*/
			...
			value:0x0 type:0x1 number:0x3  //my numble is 0x2
			*/
			...
			value:0x0 type:0x1 number:0x4   //my numble is 0x3
		 	*/
